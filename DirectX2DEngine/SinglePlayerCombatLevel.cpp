...
{
	// Spawn Actors
	std::shared_ptr<FighterCharacter<PlayerController>> player = GetWorld().SpawnActor<FighterCharacter<PlayerController>>(FVec2D(-250, -125), "Player", static_cast<FighterCharacter<PlayerController>::EFighterName>(selection.first - 1), 15);
	std::shared_ptr<FighterCharacter<DummyController>> enemy = GetWorld().SpawnActor<FighterCharacter<DummyController>>(FVec2D(250, -125), "Enemy", static_cast<FighterCharacter<DummyController>::EFighterName>(selection.second - 1), 12);
